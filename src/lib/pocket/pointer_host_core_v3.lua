...
local HOST_VERSION = "0.23.0-alpha.5.4.3"
...
function Host.run(corePath,profileName)
    ...
    local state={
        ...
        lastPrompt=nil,input=nil,entropyRemaining=0,
    }
    ...
    local function drawInputPanel()
        ...
        fillLine(native,y0,tostring(input.label or "Input").."  arrows adjust",colors.black,colors.lightGray)
    ...
    env.print=function(...)
        ...
        if joined:find("Y=CONFIRM",1,true) then setMode("confirm"); drawPanel()
    ...
    env.read=function(replaceChar,history,completeFn,defaultText)
        ...
        state.input=input; setMode("input"); drawPanel()
        local typed=""
        while true do
            ...
            if e=="key" then
                if a==keys.enter then
                    ...
                    state.input=nil; setMode("normal"); drawPanel(); return out
