...
local function normalizeConfig(value)
    ...
    for key, item in pairs(value.units) do
        ...
        if id and type(item) == "table" and type(item.session) == "string" and item.session ~= "" then
            result.units[tostring(id)] = {
                id = id,
                name = cleanString(item.name, "Unit " .. tostring(id), 48),
                label = cleanString(item.label, "", 48),
                session = item.session:sub(1, 128),
                enrolledAt = tonumber(item.enrolledAt) or Protocol.nowMs(),
                txSeq = math.max(0, math.floor(tonumber(item.txSeq) or 0)),
                lastSeen = 0,
                bootId = "",
    ...
    -- Safety invariant: every controller process starts SAFE after reboot/restart.
...
function Controller:isOnline(unit, now)
    ...
    return unit and tonumber(unit.lastSeen) and
        now - tonumber(unit.lastSeen) <= Protocol.UNIT_OFFLINE_MS
end
...
function Controller:sendPacket(recipient, messageType, payload, session, seq)
    ...
    local packet = Protocol.packet(messageType, payload, {
        session = session,
        seq = seq
    })
...
function Controller:setMode(mode)
    ...
    if not Protocol.validMode(mode) then
...
function Controller:handleEnrollRequest(sender, payload)
    ...
    if self.units[tostring(sender)] then
        return false
    end
    ...
    if not code:match("^%d%d%d%d%d%d$") or nonce == "" or #nonce > 128 then
        return false
    end
...
function Controller:confirmEnrollment(id, code)
    ...
    if code ~= nil and tostring(code) ~= pending.code then
        return false, "pairing_code_mismatch"
    end

    local session = randomHex(
        tostring(id) .. ":" .. pending.nonce .. ":" .. tostring(math.random(0, 999999))
    )
    ...
    local sent, sendError = self:sendPacket(
        id,
        "enroll_accept",
        {
            nonce = pending.nonce,
    ...
    if not sent then
        self.lastError = "enroll_accept_send_failed:" .. tostring(sendError or "")
    end
...
function Controller:handleHeartbeat(sender, packet)
    ...
    if not unit or packet.session ~= unit.session then
        return false
    end
...
function Controller:sendCommand(id, command, requestId)
    ...
    if not id then return false, "invalid_unit" end
    if not Protocol.isRemoteCommand(command) then
        return false, "command_not_allowed"
    end

    if Protocol.isCombatCommand(command) and self.mode == Protocol.MODE_SAFE then
    ...
    if not sent then
        return false, "send_failed:" .. tostring(sendError or "")
    end
