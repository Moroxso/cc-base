...
local function writeJson(path, value)
    ...
    if fs.exists(path) then
        if not pcall(fs.move, path, bak) then pcall(fs.delete, tmp); return false, "backup_failed" end
    end
    if not pcall(fs.move, tmp, path) then
...
local function firstRun()
    ...
    local cfg = {
        schema=3, fleetId=fleetId, key=key, name=name, role=role,
...
local function normalizeConfig(cfg)
    if type(cfg) ~= "table" or (cfg.schema ~= 1 and cfg.schema ~= 2 and cfg.schema ~= 3) then return nil end
    ...
    cfg.nav.x, cfg.nav.y, cfg.nav.z = tonumber(cfg.nav.x) or 0, tonumber(cfg.nav.y) or 0, tonumber(cfg.nav.z) or 0
    cfg.nav.frame = tostring(cfg.nav.frame or cfg.fleetId)
    if type(cfg.homeNav) == "table" then
        cfg.homeNav = {
            x=tonumber(cfg.homeNav.x) or 0, y=tonumber(cfg.homeNav.y) or 0, z=tonumber(cfg.homeNav.z) or 0,
...
local lastValidMesh, everLinked = 0, false
...
local function markMove(command)
    ...
    if command == "forward" then navPosition.x=navPosition.x+d.x; navPosition.z=navPosition.z+d.z
    elseif command == "back" then navPosition.x=navPosition.x-d.x; navPosition.z=navPosition.z-d.z
    elseif command == "up" then navPosition.y=navPosition.y+1
    elseif command == "down" then navPosition.y=navPosition.y-1 end
    poseDirty = true
end

local function move(command)
    local fn = command=="forward" and turtle.forward or command=="back" and turtle.back
        or command=="up" and turtle.up or command=="down" and turtle.down
...
local function distanceHome()
    ...
    return math.abs(navPosition.x-config.homeNav.x)+math.abs(navPosition.y-config.homeNav.y)+math.abs(navPosition.z-config.homeNav.z)
end
...
local function checkLowFuel()
    ...
    if fuel <= need + RTB_RESERVE then rtbActive=true; rtbReason="LOW_FUEL"; state="RTB"; return true end
    return false
end
...
local function rtbStep()
    ...
    elseif y<hy then ok,err=move("up") else ok,err=move("down") end
...
local function startTunnelJob(args, requestId)
    ...
    if not fuelOk then return false, "insufficient_fuel:" .. tostring(fuel or "?") .. "/" .. tostring(fuelNeed) end
    rtbActive=false; rtbReason=""
    activeJob = {
...
local function perform(command, args, requestId)
    ...
    autoRefuel()
    local ok,err
    if command=="forward" or command=="back" or command=="up" or command=="down" then ok,err=move(command)
...
local function acceptCommand(packet)
    ...
    if commandSeq==last then
        ...
        if cached and cached.seq==commandSeq and cached.requestId==payload.requestId then sendResult(packet,payload,cached.ok,cached.detail) end
        return
    end
    ...
    if not activeJob then state=ok and (rtbActive and "RTB" or "IDLE") or ("FAILED:"..tostring(detail)) end
...
local function handlePacket(packet,protocol)
    ...
    if packet.type=="command" then acceptCommand(packet) end
    maybeRelay(packet)
end
...
while true do
    ...
    elseif event=="timer" and a==statusTimer then
        ...
        send("status","*",{
            unit=os.getComputerID(),name=config.name,role=config.role,state=state,linkState=linkState,
            fuel=fuel,fuelLimit=fuelLimit,navPos={x=navPosition.x,y=navPosition.y,z=navPosition.z},navFrame=navPosition.frame,
